...
TEST_F(CvrptwGASolverTest, solve) {
    vrp::GASolver<cvrptw::Reader> gaSolver("data/CVRPTW/Solomon/100/c103.txt", this->nullstream);
    EXPECT_FALSE(dynamic_cast<vrp::Problem *>(gaSolver.getProblem())->clients.empty());
    EXPECT_EQ(dynamic_cast<vrp::Problem *>(gaSolver.getProblem())->clients.size(), 100);
