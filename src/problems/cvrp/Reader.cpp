...
CVRP::Problem *CVRP::Reader::readFile(std::string filepath)
{
    ...
    try {
        ...
        while (getline(fh, line)) {
            ...
            if (line.find("NODE_COORD_SECTION") != std::string::npos) {
                if (node_coord_type == TWOD_COORDS) {
                    ...
                    problem->setDepot(new Depot(1, x_depot, y_depot));
                }
        ...
        for (unsigned i = 1; i <= nbClients; ++i) {
            if (i == 0) continue;
            problem->clients.push_back(new Client(i, demand[i - 1], x[i - 1], y[i - 1]));
        }
