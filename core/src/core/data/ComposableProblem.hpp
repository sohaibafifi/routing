...
namespace routing {
    ...
    public:
        ...
        ~ComposableProblem() override {
            // Clear legacy vectors first to prevent base destructor from deleting
            // (we own the entities via unique_ptr in composable vectors)
            clients.clear();
            vehicles.clear();
            depots.clear();

            // Now clean up composable entities
            composableClients_.clear();
            composableVehicles_.clear();
            composableDepots_.clear();
        }
         ...
         *
         * Also adds to the legacy clients vector for compatibility with existing code.
         */
        ComposableClient* addClient(unsigned id) {
            ...
            // Also add to legacy vector for compatibility
         ...
         *
         * Also adds to the legacy vehicles vector for compatibility with existing code.
         */
         ...
         *
         * Also adds to the legacy depots vector for compatibility with existing code.
         */
    ...
    private:
        void refreshActiveComponents() {
            activeGenerators_ = AttributeRegistry::instance().getGenerators(enabledAttributes_);
            activeEvaluators_ = AttributeRegistry::instance().getEvaluators(enabledAttributes_);
        }
