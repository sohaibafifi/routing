...
namespace routing {
    ...
    inline void Particle::updateCurrentPosition(double inertia, double local, double global,
                                                Sequence *global_best) {
        if (!sequence || !local_best_position) {
            return;
        }

        if (global_best && global >= local && global >= inertia) {
            *sequence = *global_best;
        } else if (local >= global && local >= inertia) {
            *sequence = *local_best_position;
        }

        cost = sequence->getCost();
        if (best_cost == 0.0 || cost < best_cost) {
            *local_best_position = *sequence;
            best_cost = cost;
        }
    }

    inline void PSOSolver::initializeSwarm() {
        swarm.clear();
        sorted_swarm.clear();
        if (!problem) {
            return;
        ...
        swarm.reserve(problem->clients.size());
        for (std::size_t i = 0; i < problem->clients.size(); ++i) {
            Particle particle(problem);
            addParticleToSwarm(particle);
        }
    }

    inline bool PSOSolver::addParticleToSwarm(Particle &particle) {
        swarm.push_back(particle);
        return true;
    }

    inline bool PSOSolver::updateLocalBestSwarm(unsigned int i) {
        return i < swarm.size();
    }
}
