...
namespace Utilities {
    namespace GetOpt {
                ...
                if (currentArg[0] == '-' && currentArg.size() > 1) {
                    ...
                    if (currentArg[1] == '-') {
                        ...
                        if (convert(currentArg, anInt, dummy.flags()) == _Option::OK) {
                            if (currentArg.size() >
                                2) // if it's larger than -d (d=digit), then assume it's a negative number:
                                _add_token(currentArg,
                                           any_option_processed ? Token::UnknownYet : Token::GlobalArgument);
                            else // size == 2: it's a 1 digit negative number
                                _shortOps[currentArg[1]].token = _add_token(currentArg,
                                                                            Token::PossibleNegativeArgument);
                            ...
                            for (size_t j = 1; j < currentArg.size(); j++)
                                _shortOps[currentArg[j]].token = _add_token(std::string(currentArg, j, 1),
                                                                            Token::ShortOption);
                        }
        ...
        GETOPT_INLINE void
        GetOpt_pp::_argc_argv_to_vector(int argc, const char *const *const argv, std::vector<std::string> &args) {
