...
namespace Utilities {
    namespace GetOpt {
            ...
            Token *get_next_option_argument() const {
                if (is_last())
                    ...
                    if (next->type == UnknownYet || next->type == OptionArgument ||
                        next->type == PossibleNegativeArgument)
                        return next;
                    else
                        return NULL;
                }
        ...
        struct _Option {
            ...
            virtual Result operator()(ShortOptions &short_ops, LongOptions &long_ops, Token *first,
        ...
        template<>
        inline _Option::Result
        convert<std::string>(const std::string &s, std::string &result, std::ios::fmtflags /*flags*/) {
            result = s;
            return _Option::OK;
        }
        ...
        public:
            ...
            virtual _Option::Result
            operator()(ShortOptions &short_ops, LongOptions &long_ops, Token *first, std::ios::fmtflags flags) const {
            ...
            operator()(ShortOptions &short_ops, LongOptions &long_ops, Token *first, std::ios::fmtflags flags) const {
                ...
                while (token != NULL && !found) {
                    found = (token->type == Token::GlobalArgument || token->type == Token::UnknownYet ||
                             token->type == Token::PossibleNegativeArgument);
                    if (!found)
                        token = token->next;
                }
                if (found) {
...
// Global Option:
        ...
        class GetOpt_pp {
            ...
            static GETOPT_INLINE void
            _argc_argv_to_vector(int argc, const char *const *const argv, std::vector<std::string> &args);
        ...
        public:
            ...
            template<class T>
            inline T getopt(char short_opt, const std::string &long_opt) throw(GetOptEx) {
                T result;
            ...
            template<class T>
            inline T getopt(char short_opt, const std::string &long_opt, const T &def_value) {
                T result;
            ...
            template<class T>
            inline T getopt(char short_opt) throw(GetOptEx) {
                T result;
            ...
            template<class T>
            inline T getopt(char short_opt, const T &def_value) {
                T result;
            ...
            public:
                ...
                _iterator<Container, Adapter, OptionType> &
                operator=(const _iterator<Container, Adapter, OptionType> &other) {
                    _it = other._it;
